...
Promise.all([
    ...
    fetch("/api/v1/photos").then(x => x.json()),
    fetch("/api/v1/photos?page=2").then(x => x.json()),
])
    ...
    function search(){
        ...
        Promise.all([
            fetch("/templates/photos.mst").then(x => x.text()),
            fetch("/api/v1/photos?searching="+tag).then(x => x.json()),
            fetch("/api/v1/photos?page=2&searching="+tag).then(x => x.json()),
        ])
            .then(([templateStr,itemsData,nextData]) => {
                document.getElementById('pagenum').innerText=itemsData.pageNumber
                itemsData=itemsData.displayData
    ...
    function next(){
        ...
        Promise.all([
            fetch("/templates/photos.mst").then(x => x.text()),
            fetch("/api/v1/photos?searching="+tag+"&page="+nextpage).then(x => x.json()),
            fetch("/api/v1/photos?searching="+tag+"&page="+(parseInt(nextpage)+1)).then(x => x.json()),
        ])
            .then(([templateStr, itemsData,nextData]) => {
                document.getElementById('pagenum').innerText=itemsData.pageNumber
                itemsData=itemsData.displayData
